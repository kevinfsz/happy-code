...
const mocks = vi.hoisted(() => {
  ...
  const backendState = {
    ...
    cancelImplementation: null as null | ((sessionId: string) => Promise<void>),
    disposeCalls: 0,
...
vi.mock('./AcpBackend', () => ({
  AcpBackend: class MockAcpBackend {
    ...
    async cancel(sessionId: string) {
      ...
      if (mocks.backendState.cancelImplementation) {
        return mocks.backendState.cancelImplementation(sessionId);
      }
      for (const listener of mocks.backendState.listeners) {
...
import { runAcp, runAcpSession } from './runAcp';
...
describe('runAcp', () => {
  ...
  const consoleLines = () => mocks.mockConsoleLog.mock.calls
    ...
    .map(stripLogPrefix);
  let signalHandlers: Map<string | symbol, (...args: any[]) => any>;
  let processOnSpy: { mockRestore: () => void };
  let processOffSpy: { mockRestore: () => void };

  beforeEach(() => {
    vi.clearAllMocks();
    signalHandlers = new Map();
    processOnSpy = vi.spyOn(process, 'on').mockImplementation(((event: string | symbol, handler: (...args: any[]) => any) => {
      signalHandlers.set(event, handler);
      return process;
    }) as typeof process.on);
    processOffSpy = vi.spyOn(process, 'off').mockImplementation(((event: string | symbol, handler: (...args: any[]) => any) => {
      if (signalHandlers.get(event) === handler) {
    ...
    mocks.backendState.cancelImplementation = null;
  ...
  it('handles SIGINT with graceful session cleanup', async () => {
    const runPromise = runAcp({
      credentials: { token: 'token', encryption: { type: 'legacy', secret: new Uint8Array(32) } },
      agentName: 'opencode',
      command: 'opencode',
    ...
    await vi.waitFor(() => {
      expect(mocks.backendState.startSessionCalls).toBe(1);
      expect(signalHandlers.get('SIGINT')).toBeTypeOf('function');
    });
    ...
    expect(mocks.backendState.cancelCalls).toEqual(['acp-session-1']);
    expect(mocks.mockSession.sendSessionDeath).toHaveBeenCalledTimes(1);
    expect(mocks.mockSession.close).toHaveBeenCalledTimes(1);
    expect(processOffSpy).toHaveBeenCalledWith('SIGINT', expect.any(Function));
    expect(processOffSpy).toHaveBeenCalledWith('SIGTERM', expect.any(Function));
  });

  it('continues ACP shutdown when cancel hangs during SIGINT', async () => {
    vi.useFakeTimers();
    mocks.backendState.cancelImplementation = () => new Promise<void>(() => {});
    ...
    void signalHandlers.get('SIGINT')?.();
  ...
  it('returns switch when session-level switch RPC is invoked and returnOnSwitch is enabled', async () => {
    const runPromise = runAcpSession({
      api: {
        getOrCreateMachine: mocks.mockGetOrCreateMachine,
        getOrCreateSession: mocks.mockGetOrCreateSession,
      } as never,
      client: mocks.mockSession as never,
      agentName: 'opencode',
      command: 'opencode',
      args: ['acp'],
      path: '/repo',
    ...
    const switchHandler = mocks.sessionHandlers.get('switch');
    expect(switchHandler).toBeTypeOf('function');

    await expect(switchHandler?.({})).resolves.toBeUndefined();
