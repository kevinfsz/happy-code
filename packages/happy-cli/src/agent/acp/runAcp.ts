...
const EXIT_ABORT_TIMEOUT_MS = 2000;
...
export type RunAcpSessionResult = 'switch' | 'exit';
...
export async function runAcpSession(opts: {
  ...
  onClientSwap?: (handler: (client: ApiSessionClient) => void) => (() => void) | void;
}): Promise<RunAcpSessionResult> {
  void opts.api;
  const verbose = opts.verbose === true;
  let session = opts.client;
  let permissionHandler = new GenericAcpPermissionHandler(session, opts.agentName);
  ...
  let exitReason: RunAcpSessionResult = 'exit';
  const removeClientSwapListener = opts.onClientSwap?.((nextClient) => {
    session = nextClient;
    permissionHandler.updateSession(nextClient);
  });
  ...
  const signalSessionEnd = () => {
    ...
    try {
      ...
      logger.debug(`[${opts.agentName}] Failed to send final keepAlive(false):`, error);
    }
  };
  ...
  const requestExit = async (reason: RunAcpSessionResult = 'exit') => {
    exitReason = reason;
    shouldExit = true;
    messageQueue.close();
    ...
    try {
      await Promise.race([
        ...
        new Promise<void>((resolve) => {
          const timeout = setTimeout(resolve, EXIT_ABORT_TIMEOUT_MS);
          timeout.unref?.();
        }),
      ]);
    } catch (error) {
      logger.debug(`[${opts.agentName}] Exit abort failed:`, error);
    }
    await finalizeRun();
  };
  ...
  session.rpcHandlerManager.registerHandler('switch', async () => {
    await requestExit('switch');
  });
  registerKillSessionHandler(session.rpcHandlerManager, requestExit);
  const handleSigint = () => {
    void requestExit();
  };
  const handleSigterm = () => {
  ...
  try {
    ...
    removeClientSwapListener?.();
    await finalizeRun();
  }

  return opts.returnOnSwitch ? exitReason : 'exit';
}
...
export async function runAcp(opts: {
  ...
  const clientSwapListeners = new Set<(client: ApiSessionClient) => void>();
  const { session: initialSession, reconnectionHandle } = setupOfflineReconnection({
    ...
    onSessionSwap: (newSession) => {
      session = newSession;
      for (const listener of clientSwapListeners) {
        listener(newSession);
      }
    },
  });
  ...
  try {
    await runAcpSession({
      ...
      onClientSwap: (handler) => {
        clientSwapListeners.add(handler);
        return () => {
          clientSwapListeners.delete(handler);
        };
      },
    });
  } finally {
    ...
    try {
      ...
      logger.debug(`[${opts.agentName}] Failed to send session death:`, error);
    }
