...
describe('CodexAppServerClient sandbox integration', () => {
    ...
    const originalHappyCodexProxy = process.env.HAPPY_CODEX_PROXY;
    ...
    beforeEach(() => {
        ...
        process.env.HAPPY_CODEX_PROXY = originalHappyCodexProxy;
    ...
    it('injects HAPPY_CODEX_PROXY into the codex child process only', async () => {
        ...
        delete process.env.https_proxy;
        delete process.env.all_proxy;
        process.env.HAPPY_CODEX_PROXY = 'http://127.0.0.1:10808';
    ...
    it('does not inject proxy vars when HAPPY_CODEX_PROXY is unset', async () => {
        delete process.env.HAPPY_CODEX_PROXY;
        delete process.env.HTTP_PROXY;
        delete process.env.HTTPS_PROXY;
        delete process.env.ALL_PROXY;
        delete process.env.http_proxy;
        ...
        const spawnOptions = mockSpawn.mock.calls.at(-1)?.[2];
        expect(spawnOptions?.env).not.toHaveProperty('HTTP_PROXY');
        expect(spawnOptions?.env).not.toHaveProperty('HTTPS_PROXY');
        expect(spawnOptions?.env).not.toHaveProperty('ALL_PROXY');
        expect(spawnOptions?.env).not.toHaveProperty('http_proxy');
        expect(spawnOptions?.env).not.toHaveProperty('https_proxy');
        expect(spawnOptions?.env).not.toHaveProperty('all_proxy');
    ...
    it('warns and skips injection when HAPPY_CODEX_PROXY is invalid', async () => {
        delete process.env.HTTP_PROXY;
        delete process.env.HTTPS_PROXY;
        delete process.env.ALL_PROXY;
        delete process.env.http_proxy;
        delete process.env.https_proxy;
        delete process.env.all_proxy;
        process.env.HAPPY_CODEX_PROXY = '127.0.0.1:10808';
