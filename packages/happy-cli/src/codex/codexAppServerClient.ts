...
const SUPPORTED_CODEX_PROXY_PROTOCOLS = new Set(['http:', 'https:', 'socks5:', 'socks5h:']);

function getCodexProxyEnvOverrides(): Record<string, string> {
    const rawProxy = process.env.HAPPY_CODEX_PROXY?.trim();
    if (!rawProxy) {
        return {};
    }
    ...
    try {
        ...
        logger.warn(`[CodexAppServer] Ignoring invalid HAPPY_CODEX_PROXY value: ${rawProxy}`);
        return {};
    }

    if (!SUPPORTED_CODEX_PROXY_PROTOCOLS.has(parsed.protocol)) {
        logger.warn(
            `[CodexAppServer] Ignoring HAPPY_CODEX_PROXY with unsupported protocol: ${parsed.protocol}`,
        );
        return {};
    }
...
export class CodexAppServerClient {
    ...
    async connect(): Promise<void> {
        ...
        Object.assign(env, getCodexProxyEnvOverrides());
