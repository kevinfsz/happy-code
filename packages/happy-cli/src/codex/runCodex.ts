...
type CleanupLogger = Pick<typeof logger, 'debug'>;
type ExitLogger = Pick<typeof logger, 'debug' | 'warn'>;

type CodexCleanupOptions = {
    session: Pick<ApiSessionClient, 'sendSessionDeath' | 'flush' | 'close' | 'keepAlive'>;
    client: { disconnect: () => Promise<void> };
    reconnectionHandle?: { cancel: () => void } | null;
    happyServer: { stop: () => void };
    keepAliveInterval: NodeJS.Timeout;
    messageBuffer: { clear: () => void };
    hasTTY: boolean;
    inkInstance?: { unmount: () => void } | null;
    stdin?: Pick<NodeJS.ReadStream, 'isTTY' | 'setRawMode' | 'pause'>;
...
async function runCleanupStep(
    ...
    try {
        ...
        cleanupLogger.debug(`[codex]: ${name} failed`, error);
    }
}

export async function cleanupCodexResources({
    session,
    client,
    reconnectionHandle,
    happyServer,
...
}: CodexCleanupOptions): Promise<void> {
    cleanupLogger.debug('[codex]: Final cleanup start');

    clearInterval(keepAliveInterval);
    cleanupLogger.debug('[codex]: clearInterval(keepAlive)');

    try {
        session.keepAlive(false, 'remote');
    } catch (error) {
        cleanupLogger.debug('[codex]: final keepAlive(false) failed', error);
    }

    if (reconnectionHandle) {
        cleanupLogger.debug('[codex]: Cancelling offline reconnection');
        reconnectionHandle.cancel();
    }

    try {
        cleanupLogger.debug('[codex]: sendSessionDeath');
        session.sendSessionDeath();
    } catch (error) {
        cleanupLogger.debug('[codex]: sendSessionDeath failed', error);
    }

    await runCleanupStep('session.flush', () => session.flush(), cleanupTimeoutMs, cleanupLogger);
    await runCleanupStep('session.close', () => session.close(), cleanupTimeoutMs, cleanupLogger);
    await runCleanupStep('client.disconnect', () => client.disconnect(), cleanupTimeoutMs, cleanupLogger);

    cleanupLogger.debug('[codex]: happyServer.stop');
    happyServer.stop();

    if (stdin.isTTY) {
        cleanupLogger.debug('[codex]: setRawMode(false)');
        try { stdin.setRawMode(false); } catch { }
    }
    if (hasTTY) {
        cleanupLogger.debug('[codex]: stdin.pause()');
        try { stdin.pause(); } catch { }
    }
    if (inkInstance) {
        cleanupLogger.debug('[codex]: inkInstance.unmount()');
        inkInstance.unmount();
    }
    messageBuffer.clear();
    cleanupLogger.debug('[codex]: Final cleanup completed');
}

type RequestCodexExitOptions = {
    beginExit: () => boolean;
...
export function requestCodexExit({
    beginExit,
    handleAbort,
    forceDisconnect,
    exitTimeoutMs = 5000,
    exitLogger = logger,
    onForceDisconnect,
}: RequestCodexExitOptions): void {
    const startedExit = beginExit();
    if (!startedExit) {
        return;
    }
    ...
    const exitTimeout = setTimeout(() => {
        ...
        exitLogger.warn(
            `[codex]: Exit abort timed out after ${exitTimeoutMs}ms; forcing backend disconnect`,
        );
        onForceDisconnect?.();
        void forceDisconnect().catch((error) => {
            exitLogger.debug('[codex]: Forced disconnect failed', error);
        });
    }, exitTimeoutMs);
    exitTimeout.unref?.();
    ...
    void handleAbort()
        .catch((error) => {
            exitLogger.debug('[codex]: Exit abort failed', error);
        })
        .finally(() => {
            exitSettled = true;
...
export async function runCodex(opts: {
    ...
    let forcedDisconnectInFlight: Promise<void> | null = null;
    ...
    const beginExit = () => {
        ...
        try {
            ...
            logger.debug('[codex]: keepAlive(false) during exit failed', error);
        }
    ...
    const forceDisconnect = async () => {
        if (!forcedDisconnectInFlight) {
            forcedDisconnectInFlight = (async () => {
                try {
                    await client.disconnect();
                } finally {
                    forcedDisconnectInFlight = null;
                }
            })();
        }

        await forcedDisconnectInFlight;
    };

    const requestExit = (source: string) => {
        logger.debug(`[codex]: Exit requested via ${source}`);
        if (exitRequested) {
            void forceDisconnect().catch((error) => {
                logger.debug('[codex]: Forced disconnect after repeated exit request failed', error);
            });
            return;
        }
        ...
        requestCodexExit({
            ...
            forceDisconnect,
            onForceDisconnect: () => {
                messageBuffer.addMessage('Codex exit timed out, forcing shutdown...', 'status');
            },
        });
    };

    const handleSigint = () => requestExit('SIGINT');
    const handleSigterm = () => requestExit('SIGTERM');
