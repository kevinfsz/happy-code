...
function processExists(pid: number): boolean {
    try {
        // process.kill with signal 0 checks if process exists without actually sending a signal
        process.kill(pid, 0);
        return true;
    } catch {
        return false;
    ...
    for (const { pid, command } of runawayProcesses) {
        try {
            ...
            if (!processExists(pid)) {
                console.log(`Process PID ${pid} already gone`);
                killed++;
                continue;
            }
            ...
            if (process.platform === 'win32') {
                ...
                // Check if still alive using signal 0
                if (processExists(pid)) {
            ...
            // Don't report ESRCH as an error - process already gone
            if (errorMessage.includes('ESRCH') || errorMessage.includes('not found')) {
                console.log(`Process PID ${pid} already gone`);
                killed++;
            } else {
